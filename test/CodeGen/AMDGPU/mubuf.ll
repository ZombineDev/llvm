...
; CHECK: buffer_load_dword v{{[0-9+]}}, v{{[0-9+]}}, s[{{[0-9]+}}:{{[0-9]+}}], 0 offen
define amdgpu_ps void @s_buffer_load(<4 x i32> addrspace(2)* inreg, <16 x i8> addrspace(2)* inreg, <32 x i8> addrspace(2)* inreg, i32 inreg, <2 x i32>, <2 x i32>, <2 x i32>, <3 x i32>, <2 x i32>, <2 x i32>, <2 x i32>, float, float, float, float, float, float, float, float, float, <4 x i32> addrspace(2)* inreg %in) {
main_body:
  %tid = call i32 @llvm.amdgcn.mbcnt.lo(i32 -1, i32 0)
  %20 = getelementptr <4 x i32>, <4 x i32> addrspace(2)* %0, i32 0
  %21 = load <4 x i32>, <4 x i32> addrspace(2)* %20
  %22 = call float @llvm.SI.load.const.v4i32(<4 x i32> %21, i32 %tid)
  %s.buffer.rsrc = load <4 x i32>, <4 x i32> addrspace(2)* %in
  %s.buffer = call i32 @llvm.amdgcn.s.buffer.load.i32(<4 x i32> %s.buffer.rsrc, i32 %tid, i1 false)
  %s.buffer.float = bitcast i32 %s.buffer to float
  call void @llvm.amdgcn.exp.f32(i32 0, i32 15, float %22, float %s.buffer.float, float %s.buffer.float, float %s.buffer.float, i1 true, i1 true)
  ret void
}
...
declare i32 @llvm.amdgcn.s.buffer.load.i32(<4 x i32> nocapture, i32, i1)
